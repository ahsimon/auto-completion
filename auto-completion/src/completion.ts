  ...
  implements InlineCompletionItemProvider
{
  ...
  private async tryParseDocument(document: TextDocument) {
    try {
      if (!this._position || !this._document) {
        return;
      }
    ...
    try {
      ...
      if (
        stopWords.some((stopWord: string) =>
          this._completion.includes(stopWord)
        )
      ) {
  ...
  private async getPrompt(prefixSuffix: PrefixSuffix) {
    ...
    if (!this._document || !this._position || !this._provider) {
      return "";
    }
